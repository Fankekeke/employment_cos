  ...
  methods: {
    handlerClosed (localPoint) {
      ...
      if (localPoint !== null && localPoint !== undefined) {
        ...
        let venue = baiduMap.getvenue(localPoint)
        venue.getLocation(localPoint, (rs) => {
          if (rs != null) {
            if (rs.venue !== undefined && rs.venue.length !== 0) {
              this.stayvenue = rs.venue
            }
            if (rs.surroundingPois !== undefined) {
              if (rs.surroundingPois.venue !== undefined && rs.surroundingPois.venue.length !== 0) {
                this.stayvenue = rs.surroundingPois.venue
              }
            }
            let obj = {}
            obj['venue'] = this.stayvenue
    ...
    setFormValues ({...venue}) {
      this.rowId = venue.id
      let fields = ['venue', 'province', 'city', 'area', 'contactPerson', 'contactMethod', 'longitude', 'latitude']
      let obj = {}
